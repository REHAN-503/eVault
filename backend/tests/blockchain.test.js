...
describe('Blockchain Service API', () => {
  ...
  beforeEach(() => {
    ...
    userRepository.findById.mockImplementation(async (id) => {
      if (id === validOwnerId) return { id, role: 'LAWYER' };
      if (id === validTargetUserId) return { id, role: 'CLIENT' };
      return null;
    });
  });
  ...
  describe('Validation', () => {
    it('should throw validation error on invalid UUIDs', async () => {
      await expect(blockchainService.recordDocument('invalid', dummyHash, dummyMetadata, validOwnerId))
        .rejects.toThrow('Invalid document ID format');
      await expect(blockchainService.recordDocument(validDocId, dummyHash, dummyMetadata, 'invalid'))
    ...
    it('should throw validation error on invalid permissions', async () => {
      ...
      await expect(blockchainService.grantAccess(validDocId, validTargetUserId, 'INVALID_PERM'))
        .rejects.toThrow('Invalid permission type');
    });

    it('should throw validation error on invalid roles during registration', async () => {
      await expect(blockchainService.registerUser(validOwnerId, 'SUPER_USER'))
  ...
  describe('Automatic Registration', () => {
    it('should auto-register user on recordDocument if not registered', async () => {
      const result = await blockchainService.recordDocument(validDocId, dummyHash, dummyMetadata, validOwnerId);
    ...
    it('should fail auto-registration if user does not exist in repository', async () => {
      ...
      await expect(blockchainService.recordDocument(validDocId, dummyHash, dummyMetadata, unknownUserId))
  ...
  describe('Duplicate Handling', () => {
    it('should throw conflict on duplicate registration', async () => {
      ...
      await expect(blockchainService.registerUser(validOwnerId, 'LAWYER'))
        .rejects.toThrow('User is already registered on the blockchain');
    });

    it('should throw conflict on duplicate document recording', async () => {
      await blockchainService.recordDocument(validDocId, dummyHash, dummyMetadata, validOwnerId);
      await expect(blockchainService.recordDocument(validDocId, 'newhash', dummyMetadata, validOwnerId))
    ...
    it('should throw conflict on duplicate grant', async () => {
      ...
      await expect(blockchainService.grantAccess(validDocId, validTargetUserId, 'READ'))
  ...
  describe('Missing / Revoke Handling', () => {
    ...
    it('should throw error on revoking non-existent access', async () => {
      await blockchainService.recordDocument(validDocId, dummyHash, dummyMetadata, validOwnerId);
      await expect(blockchainService.revokeAccess(validDocId, validTargetUserId))
        .rejects.toThrow('Access not found to revoke');
    });
  ...
  describe('Document Updates and History', () => {
    ...
    it('should throw error when updating missing document', async () => {
      await expect(blockchainService.updateDocument(validDocId, 'hash', { version: 2 }))
