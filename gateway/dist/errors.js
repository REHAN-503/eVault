...
exports.translateChaincodeError = translateChaincodeError;
...
function extractCandidateMessages(err) {
    const messages = [];
    if (!err)
        return messages;
...
function translateChaincodeError(err) {
    ...
    for (const msg of candidates) {
        ...
        if (match) {
            ...
            const wrapped = new Error(cleanMessage);
    ...
    const fallbackMessage = candidates[0] ?? (err instanceof Error ? err.message : String(err));
